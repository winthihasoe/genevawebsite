...
export default function ElderCareTopics(props) {
    const elderSkillTopics = props.elderSkillTopics;
    ...
    const handleNewSkill = (e) => {
        e.preventDefault();
        post(route('addNewElderSkill'), data.newSkill);
        setData({newSkill: ""});
    }

    function handleDelete(id) {
        destroy(route('destroyElderSkill', id));
    }
   

  return (
    <Authenticated auth={props.auth} errors={props.errors}>
        <Head title='Elder Skills' />
        <Container maxWidth='md'>
            <Typography variant='h4'>Showing Elder Care Topics</Typography>
