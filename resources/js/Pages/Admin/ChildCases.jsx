...
import { Container, Table, Typography, Paper, TableBody, TableCell, TableContainer, TableHead, TableRow, Box } from '@mui/material';
    ...
    return (
        <Authenticated auth={props.auth}>
            <Container maxWidth='lg'>
                ...
                { childCases.length == 0 ? <Box sx={{ margin: '20px auto' }}><Typography textAlign='center' variant='h3'>No Child Case right now!</Typography></Box> : 
                    ...
                    <Table sx={{ minWidth: 650}} aria-label="child case table">
                        ...
                        {childCases.map((childCase, index) => (
                            <TableRow key={childCase.id}>
                                ...
                                <TableCell align="center">{childCase.caregiver_name}</TableCell>
                                <TableCell align="center">{childCase.duty}</TableCell>
                                <TableCell align='center'>{childCase.booking_created_at}</TableCell>
                                <TableCell align='center'>{childCase.location}</TableCell>
                            </TableRow>
                        ))}
                        </TableBody>
                    </Table>
